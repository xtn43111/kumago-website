...
// Messages are sent as Telegram HTML so the long map URL can hide behind a "地點"
// link. <url> is an internal sentinel for "this line is a map link".
const MAP_MARK = "";
...
function renderLine(line) {
  if (line.startsWith(MAP_MARK)) {
    return `📍 <a href="${escapeAttr(line.slice(1))}">Google map連結</a>`;
  }
...
function buildOrderPush(v) {
  ...
  const lines = [
    ...
    v.mapUrl ? `地圖：<a href="${escapeAttr(v.mapUrl)}">Google 地圖</a>` : null,
