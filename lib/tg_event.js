...
/* Is this message a "replace the photo on an EXISTING event" command?
 * The owner attaches new photo(s) and captions them with an update keyword,
 * a date, and (optionally) a title keyword to pick the event:
 *
 *   更新照片：7/5 庭綺          (single line: date then keyword)
 *   更新照片\n日期：7/5\n標題：庭綺   (labelled lines also work)
 *
 * Aliases: 更新照片 / 換照片 / 换照片 / 改照片 / 照片更新.
 * Returns null when the text is not an update command at all; otherwise
 *   { ok:true, date:"YYYY-MM-DD", keyword }  or  { ok:false, error }.  */
const PHOTO_UPDATE_RE = /^\/?(更新照片|換照片|换照片|改照片|照片更新)\s*[：:]?\s*/;
function parsePhotoUpdate(text, todayJst) {
  ...
  if (!dateRaw) {
    return { ok: false, error: "請帶日期，例：更新照片：7/5 庭綺" };
  }
...
module.exports = {
  ...
  photoDescLine, PHOTO_LINE_RE, parsePhotoUpdate,
};
