...
/* 客人確認信收件地址：contact 是 email 就用它；不是（電話、LINE 名稱等）就
 * fallback 用 Stripe 結帳頁客人自填的 email。付款完成一律要寄——兩個都拿不到
 * 才回 null，由呼叫端記成錯誤，讓老闆在 Telegram 警告裡看到。 */
const EMAIL_RE = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
function resolveCustomerEmail(contact, fallbackEmail) {
  const c = (contact || "").trim();
  if (EMAIL_RE.test(c)) return c;
  const f = (fallbackEmail || "").trim();
  if (EMAIL_RE.test(f)) return f;
  return null;
}

/* 續租版寄信：老闆一封、客人一封。同 sendOrderEmails 的韌性契約：永不 throw，
 * 回報告。 */
async function sendRenewalEmails(v, fallbackEmail) {
  ...
  const to = resolveCustomerEmail(v.contact, fallbackEmail);
  if (to) {
    try { await send(to, buildRenewalCustomerEmail(v)); report.customer = true; }
    catch (e) { report.errors.push("customer: " + e.message); }
  } else {
    report.errors.push("customer: 找不到客人 email（contact 非 email 且結帳頁未留 email），確認信未寄");
  }
  return report;
}

/* =========================== MOVING EMAILS =========================== */
/* v = lib/moving_payment.js movingView()：name/phone/email/movingDate/
 * movingTime/addressFrom/addressTo/elevator/items/note/lang/amount。
 * 客人信 zh/en 按 v.lang；老闆信固定中文。 */

function buildMovingCustomerEmail(v) {
  const en = (v.lang || "zh") === "en";
  const T = en
    ? {
        tagline: "Furniture & Appliance Rental ・ Moving Service",
        title: "Payment received!",
        greet: `Hello ${esc(v.name)}, thank you — we've received your moving service payment 🌿`,
        section: "YOUR MOVING SERVICE",
        item: "Moving service fee",
        total: "Total (paid)",
        infoTitle: "Moving details",
        date: "Moving date",
        time: "Time",
        from: "From",
        to: "To",
        items: "Items",
        next: "We'll contact you before the moving day to confirm the exact schedule.",
        lineBtn: "💬 Questions? Reach us on LINE",
        footer1: "Keeping every home warm 🐻",
        footer2: "This is an automated email ・ KUMAGO Furniture & Appliance Rental",
        subject: `【KUMAGO】Moving Service Confirmation — ${v.movingDate}`,
      }
    : {
        tagline: "家具家電租賃 ・ 搬家服務",
        title: "已收到款項！",
        greet: `${esc(v.name)} 您好，已收到您的搬家服務費，感謝您 🌿`,
        section: "YOUR MOVING SERVICE",
        item: "搬家服務費",
        total: "合計（已付款）",
        infoTitle: "搬家資訊",
        ...
        to: "搬入地址",
        items: "品項",
        next: "我們將於搬家日前與您確認詳細時間。",
        lineBtn: "💬 有問題？官方 LINE 找我們",
        footer1: "守護每一個溫暖的家 🐻",
        footer2: "此信為系統自動寄送　・　KUMAGO 家具家電租賃",
        subject: `【KUMAGO】搬家服務確認 — ${v.movingDate}`,
      };

  const html = `<!DOCTYPE html>
<html lang="${en ? "en" : "zh-Hant"}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:${BRAND.offwhite};font-family:-apple-system,'PingFang TC','Microsoft JhengHei',sans-serif;color:${BRAND.charcoal};">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="background:linear-gradient(135deg,${BRAND.green} 0%,${BRAND.greenDeep} 100%);padding:34px 28px;text-align:center;">
          <div style="font-size:26px;font-weight:bold;color:#ffffff;letter-spacing:1px;">KUMAGO 🐻🏠</div>
          <div style="color:#eaf7d8;font-size:14px;margin-top:6px;">${T.tagline}</div>
        </td></tr>

        <tr><td style="padding:30px 28px 6px;text-align:center;">
          <div style="font-size:50px;line-height:1;">🚚</div>
          <h1 style="font-size:24px;margin:12px 0 4px;color:${BRAND.greenDeep};">${T.title}</h1>
          <p style="font-size:15px;color:#666;margin:0;">${T.greet}</p>
        </td></tr>

        <tr><td style="padding:18px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};border:1px solid ${BRAND.cream};border-radius:16px;">
            <tr><td style="padding:22px 24px;">
              <div style="font-size:13px;color:#a99;letter-spacing:2px;margin-bottom:10px;">${T.section}</div>
              ${row(T.item, yen(v.amount))}
              <div style="border-top:1px dashed #d8cfbe;margin:14px 0;"></div>
              ${row(T.total, yen(v.amount), { color: BRAND.greenDeep, bold: true })}
            </td></tr>
          </table>
        </td></tr>
        ...
        <tr><td style="padding:6px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f8e6;border-radius:14px;">
            <tr><td style="padding:18px 22px;font-size:14px;line-height:1.9;color:${BRAND.charcoal};">
              🚚 <b>${T.infoTitle}</b><br>
              ${T.date}：${esc(v.movingDate)}${v.movingTime ? "　" + esc(v.movingTime) : ""}<br>
              ${T.from}：${esc(v.addressFrom)}<br>
              ${v.addressTo ? `${T.to}：${esc(v.addressTo)}<br>` : ""}${v.items ? `${T.items}：${esc(v.items)}<br>` : ""}${T.next}
            </td></tr>
          </table>
        </td></tr>

        <tr><td style="padding:18px 28px 6px;text-align:center;">
          <a href="${LINE_URL}" style="display:inline-block;padding:11px 20px;background:#06c755;color:#fff;text-decoration:none;border-radius:999px;font-weight:bold;">${T.lineBtn}</a>
        </td></tr>

        <tr><td style="padding:22px 28px 30px;text-align:center;">
          <div style="font-size:16px;color:#444;">${T.footer1}</div>
          <div style="font-size:12px;color:#aaa;margin-top:14px;">${T.footer2}</div>
        </td></tr>
  ...
  const text = en
    ? `KUMAGO Moving Service — payment received!\n\nHello ${v.name}, thank you.\n\n` +
      `Moving service fee: ${yen(v.amount)} (paid)\n\n` +
      `Moving date: ${v.movingDate}${v.movingTime ? " " + v.movingTime : ""}\n` +
      `From: ${v.addressFrom}\n` +
      (v.addressTo ? `To: ${v.addressTo}\n` : "") +
      (v.items ? `Items: ${v.items}\n` : "") +
      `\n${T.next}\n\nLINE: ${LINE_URL}\n\nKUMAGO Furniture & Appliance Rental`
    : `KUMAGO 搬家服務 已收到款項！\n\n${v.name} 您好，已收到您的搬家服務費，感謝您。\n\n` +
      `搬家服務費：${yen(v.amount)}（已付款）\n\n` +
      `搬家日：${v.movingDate}${v.movingTime ? " " + v.movingTime : ""}\n` +
      `搬出地址：${v.addressFrom}\n` +
      (v.addressTo ? `搬入地址：${v.addressTo}\n` : "") +
      (v.items ? `品項：${v.items}\n` : "") +
      `\n我們將於搬家日前與您確認詳細時間。\n\n官方 LINE：${LINE_URL}\n\nKUMAGO 家具家電租賃`;
...
function buildMovingOwnerEmail(v) {
  const elevatorLabel =
    v.elevator === "yes" ? "有" : v.elevator === "no" ? "無" : "（未填）";
  const text =
    `🚚 KUMAGO 搬家服務費已付款！\n\n` +
    `金額（已付款）：${yen(v.amount)}\n` +
    `搬家日：${v.movingDate}${v.movingTime ? "　" + v.movingTime : ""}\n` +
    `搬出：${v.addressFrom}\n` +
    (v.addressTo ? `搬入：${v.addressTo}\n` : "") +
    `電梯：${elevatorLabel}\n` +
    (v.items ? `品項：${v.items}\n` : "") +
    `\n── 客人 ──\n` +
    `姓名：${v.name}\n` +
    (v.lineName ? `LINE 名稱：${v.lineName}\n` : "") +
    (v.phone ? `電話：${v.phone}\n` : "") +
    (v.email ? `email：${v.email}\n` : "") +
    (v.note ? `\n── 備註 ──\n${v.note}\n` : "");

  const html =
    `<h2 style="color:${BRAND.greenDeep};font-family:sans-serif;">🚚 KUMAGO 搬家服務費已付款</h2>` +
    `<pre style="font-size:14px;line-height:1.75;font-family:-apple-system,'PingFang TC',monospace;background:${BRAND.offwhite};padding:16px 18px;border-radius:12px;border:1px solid ${BRAND.cream};white-space:pre-wrap;">${esc(text)}</pre>`;

  const subject = `🚚 搬家服務 — ${v.movingDate}（${v.name}）${yen(v.amount)}`;
  return { subject, html, text };
}

/* 搬家版寄信：老闆一封、客人一封。客人 email 拿不到（metadata 沒填、結帳頁
 * 也沒留）→ skip 不算錯（搬家/回收客人聯絡管道以 LINE 為主）。永不 throw。 */
async function sendMovingEmails(v, fallbackEmail) {
  const report = { owner: false, customer: false, skipped: false, errors: [] };
  if (!isConfigured()) {
    report.skipped = true;
    return report;
  ...
  const owner = (process.env.OWNER_EMAIL || "").trim();
  if (owner) {
    try { await send(owner, buildMovingOwnerEmail(v)); report.owner = true; }
    catch (e) { report.errors.push("owner: " + e.message); }
  }
  const to = resolveCustomerEmail(v.email, fallbackEmail);
  if (to) {
    try { await send(to, buildMovingCustomerEmail(v)); report.customer = true; }
    catch (e) { report.errors.push("customer: " + e.message); }
  } else {
    report.customerSkipped = "no_email"; // 沒 email 不算錯，不進 errors
  }
  return report;
}

/* ========================== RECOVERY EMAILS ========================== */
/* v = lib/recovery_payment.js recoveryView()：name/phone/email/address/
 * recoveryDate/items/elevator/note/amount。回收指令目前只出中文。 */

function buildRecoveryCustomerEmail(v) {
  const html = `<!DOCTYPE html>
<html lang="zh-Hant"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:${BRAND.offwhite};font-family:-apple-system,'PingFang TC','Microsoft JhengHei',sans-serif;color:${BRAND.charcoal};">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="background:linear-gradient(135deg,${BRAND.green} 0%,${BRAND.greenDeep} 100%);padding:34px 28px;text-align:center;">
          <div style="font-size:26px;font-weight:bold;color:#ffffff;letter-spacing:1px;">KUMAGO 🐻🏠</div>
          <div style="color:#eaf7d8;font-size:14px;margin-top:6px;">家具家電租賃 ・ 期滿回收</div>
        </td></tr>

        <tr><td style="padding:30px 28px 6px;text-align:center;">
          <div style="font-size:50px;line-height:1;">♻️</div>
          <h1 style="font-size:24px;margin:12px 0 4px;color:${BRAND.greenDeep};">已收到款項！</h1>
          <p style="font-size:15px;color:#666;margin:0;">${esc(v.name)} 您好，已收到您的回收處理費，感謝您 🌿</p>
        </td></tr>

        <tr><td style="padding:18px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};border:1px solid ${BRAND.cream};border-radius:16px;">
            <tr><td style="padding:22px 24px;">
              <div style="font-size:13px;color:#a99;letter-spacing:2px;margin-bottom:10px;">YOUR PAYMENT</div>
              ${row("期滿回收處理費", yen(v.amount))}
              <div style="border-top:1px dashed #d8cfbe;margin:14px 0;"></div>
              ${row("合計（已付款）", yen(v.amount), { color: BRAND.greenDeep, bold: true })}
            </td></tr>
          </table>
        </td></tr>
        ...
        <tr><td style="padding:6px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f8e6;border-radius:14px;">
            <tr><td style="padding:18px 22px;font-size:14px;line-height:1.9;color:${BRAND.charcoal};">
              ♻️ <b>回收資訊</b><br>
              回收日：${esc(v.recoveryDate)}<br>
              收取地址：${esc(v.address)}<br>
              ${v.items ? `品項：${esc(v.items)}<br>` : ""}回收當天若時間有異動會再與您聯繫。
            </td></tr>
          </table>
        </td></tr>
  ...
  const text =
    `KUMAGO 期滿回收 已收到款項！\n\n${v.name} 您好，已收到您的回收處理費，感謝您。\n\n` +
    `期滿回收處理費：${yen(v.amount)}（已付款）\n\n` +
    `回收日：${v.recoveryDate}\n` +
    `收取地址：${v.address}\n` +
    (v.items ? `品項：${v.items}\n` : "") +
    `\n回收當天若時間有異動會再與您聯繫。\n\n官方 LINE：${LINE_URL}\n\nKUMAGO 家具家電租賃`;

  const subject = `【KUMAGO】回收處理費 付款確認 — ${v.recoveryDate}`;
  return { subject, html, text };
}

function buildRecoveryOwnerEmail(v) {
  const elevatorLabel =
    v.elevator === "yes" ? "有" : v.elevator === "no" ? "無" : "（未填）";
  const text =
    `♻️ KUMAGO 回收處理費已付款！\n\n` +
  ...
  const html =
    `<h2 style="color:${BRAND.greenDeep};font-family:sans-serif;">♻️ KUMAGO 回收處理費已付款</h2>` +
    `<pre style="font-size:14px;line-height:1.75;font-family:-apple-system,'PingFang TC',monospace;background:${BRAND.offwhite};padding:16px 18px;border-radius:12px;border:1px solid ${BRAND.cream};white-space:pre-wrap;">${esc(text)}</pre>`;

  const subject = `♻️ 回收處理費 — ${v.recoveryDate}（${v.name}）${yen(v.amount)}`;
  return { subject, html, text };
}

/* 回收版寄信：老闆一封、客人一封。客人 email 拿不到 → skip 不算錯
 *（回收客人聯絡管道是 LINE）。永不 throw。 */
async function sendRecoveryEmails(v, fallbackEmail) {
  const report = { owner: false, customer: false, skipped: false, errors: [] };
  if (!isConfigured()) {
    report.skipped = true;
    return report;
  ...
  const owner = (process.env.OWNER_EMAIL || "").trim();
  if (owner) {
    try { await send(owner, buildRecoveryOwnerEmail(v)); report.owner = true; }
    catch (e) { report.errors.push("owner: " + e.message); }
  }
  const to = resolveCustomerEmail(v.email, fallbackEmail);
  if (to) {
    try { await send(to, buildRecoveryCustomerEmail(v)); report.customer = true; }
    catch (e) { report.errors.push("customer: " + e.message); }
  } else {
    report.customerSkipped = "no_email"; // 沒 email 不算錯，不進 errors
  }
...
/* ======================= 買斷（家電傢俱）EMAIL ======================= */
function buildBuyoutCustomerEmail(v) {
  const html = `<!DOCTYPE html>
<html lang="zh-Hant"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:${BRAND.offwhite};font-family:-apple-system,'PingFang TC','Microsoft JhengHei',sans-serif;color:${BRAND.charcoal};">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="background:linear-gradient(135deg,${BRAND.green} 0%,${BRAND.greenDeep} 100%);padding:34px 28px;text-align:center;">
          <div style="font-size:26px;font-weight:bold;color:#ffffff;letter-spacing:1px;">KUMAGO 🐻🏠</div>
          <div style="color:#eaf7d8;font-size:14px;margin-top:6px;">家具家電租賃 ・ 買斷</div>
        </td></tr>

        <tr><td style="padding:30px 28px 6px;text-align:center;">
          <div style="font-size:50px;line-height:1;">🛋️</div>
          <h1 style="font-size:24px;margin:12px 0 4px;color:${BRAND.greenDeep};">已收到款項！</h1>
          <p style="font-size:15px;color:#666;margin:0;">${esc(v.name)} 您好，已收到您的買斷款項，感謝您 🌿</p>
        </td></tr>
        ...
        <tr><td style="padding:18px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.offwhite};border:1px solid ${BRAND.cream};border-radius:16px;">
            <tr><td style="padding:22px 24px;">
              <div style="font-size:13px;color:#a99;letter-spacing:2px;margin-bottom:10px;">YOUR PAYMENT</div>
              ${row(v.items ? "買斷：" + esc(v.items) : "家電傢俱買斷", yen(v.amount))}
              <div style="border-top:1px dashed #d8cfbe;margin:14px 0;"></div>
              ${row("合計（已付款）", yen(v.amount), { color: BRAND.greenDeep, bold: true })}
            </td></tr>
          </table>
        ...
        <tr><td style="padding:6px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f8e6;border-radius:14px;">
            <tr><td style="padding:18px 22px;font-size:14px;line-height:1.9;color:${BRAND.charcoal};">
              🛋️ <b>買斷資訊</b><br>
              ${v.items ? `品項：${esc(v.items)}<br>` : ""}此款項為家電傢俱買斷，付清後該物品歸您所有，無需再回收。
            </td></tr>
          </table>
        </td></tr>
  ...
  const text =
    `KUMAGO 買斷 已收到款項！\n\n${v.name} 您好，已收到您的買斷款項，感謝您。\n\n` +
    `${v.items ? "買斷品項：" + v.items + "\n" : ""}買斷金額：${yen(v.amount)}（已付款）\n\n` +
    `付清後該物品歸您所有，無需再回收。\n\n官方 LINE：${LINE_URL}\n\nKUMAGO 家具家電租賃`;

  const subject = `【KUMAGO】買斷 付款確認 — ${yen(v.amount)}`;
  return { subject, html, text };
}

function buildBuyoutOwnerEmail(v) {
  const text =
    `🛋️ KUMAGO 買斷款項已付款！\n\n` +
    `金額（已付款）：${yen(v.amount)}\n` +
    (v.items ? `買斷品項：${v.items}\n` : "") +
  ...
  const html =
    `<h2 style="color:${BRAND.greenDeep};font-family:sans-serif;">🛋️ KUMAGO 買斷款項已付款</h2>` +
    `<pre style="font-size:14px;line-height:1.75;font-family:-apple-system,'PingFang TC',monospace;background:${BRAND.offwhite};padding:16px 18px;border-radius:12px;border:1px solid ${BRAND.cream};white-space:pre-wrap;">${esc(text)}</pre>`;

  const subject = `🛋️ 買斷 — ${v.name} ${yen(v.amount)}`;
  return { subject, html, text };
}

/* 買斷版寄信：老闆一封、客人一封。客人 email 拿不到 → skip 不算錯。永不 throw。 */
async function sendBuyoutEmails(v, fallbackEmail) {
  const report = { owner: false, customer: false, skipped: false, errors: [] };
  if (!isConfigured()) {
    report.skipped = true;
    return report;
  ...
  const owner = (process.env.OWNER_EMAIL || "").trim();
  if (owner) {
    try { await send(owner, buildBuyoutOwnerEmail(v)); report.owner = true; }
    catch (e) { report.errors.push("owner: " + e.message); }
  }
  const to = resolveCustomerEmail(v.email, fallbackEmail);
  if (to) {
    try { await send(to, buildBuyoutCustomerEmail(v)); report.customer = true; }
    catch (e) { report.errors.push("customer: " + e.message); }
  } else {
    report.customerSkipped = "no_email";
  }
  return report;
}
...
async function sendOrderEmails(meta, lineItems, amountTotal, fallbackEmail) {
  const report = { owner: false, customer: false, skipped: false, errors: [] };
  if (!isConfigured()) {
    report.skipped = true;
    return report;
  ...
  const to = resolveCustomerEmail(v.contact, fallbackEmail);
  if (to) {
    try { await send(to, buildCustomerEmail(v)); report.customer = true; }
    catch (e) { report.errors.push("customer: " + e.message); }
  } else {
    report.errors.push("customer: 找不到客人 email（contact 非 email 且結帳頁未留 email），確認信未寄");
  }
...
module.exports = {
  ...
  buildMovingCustomerEmail, buildMovingOwnerEmail, sendMovingEmails,
  buildRecoveryCustomerEmail, buildRecoveryOwnerEmail, sendRecoveryEmails,
  buildBuyoutCustomerEmail, buildBuyoutOwnerEmail, sendBuyoutEmails,
