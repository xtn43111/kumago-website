...
const ADDON_NAME = {
  ...
  clothes_rack: "室內掛衣架", desk: "桌椅組", rice_cooker: "電飯鍋",
  pot: "鍋具組", clothesline: "曬衣桿",
};
...
function buildCustomerEmail(v) {
  const itemRows = v.items
    .map((i) => row(i.label, yen(i.amount)))
    .join("");
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:30px 28px 6px;text-align:center;">
          ...
          <p style="font-size:15px;color:#666;margin:0;">${v.name} 您好，感謝您選擇 KUMAGO，我們會盡快為您安排配送 🌿</p>
        </td></tr>
        ...
        <tr><td style="padding:6px 28px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f8e6;border-radius:14px;">
            <tr><td style="padding:18px 22px;font-size:14px;line-height:1.9;color:${BRAND.charcoal};">
              📦 <b>配送資訊</b><br>
              配送地區：${v.areaName}<br>
              配送日：${v.moveInDate}　${v.deliveryTime}<br>
              地址：${fullAddr}<br>
              電梯：${v.elevator}　加購：${addonLine}${v.note ? `<br>備註：${v.note}` : ""}
            </td></tr>
          </table>
        </td></tr>
...
function buildOwnerEmail(v) {
  ...
  const html =
    ...
    `<pre style="font-size:14px;line-height:1.75;font-family:-apple-system,'PingFang TC',monospace;background:${BRAND.offwhite};padding:16px 18px;border-radius:12px;border:1px solid ${BRAND.cream};white-space:pre-wrap;">${text}</pre>` +
    (v.mapUrl ? `<p><a href="${v.mapUrl}" style="color:${BRAND.greenDeep};">📍 在 Google 地圖開啟配送位置</a></p>` : "");
