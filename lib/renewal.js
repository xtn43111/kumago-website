...
const { sendLinePush, buildRenewalPaidLineText } = require("./line_push.js");
...
async function handleRenewal(session, meta, lineItems, amountTotal) {
  ...
  // 3. 確認信（老闆＋客人）。contact 非 email（電話/LINE 名稱）時 fallback 用
  // Stripe 結帳頁客人自填的 email——付款完成一律要寄。
  try {
    const checkoutEmail =
      (session.customer_details && session.customer_details.email) || "";
    report.emails = await sendRenewalEmails(v, checkoutEmail);
  } catch (e) {
    report.emails.errors.push(e.message);
  }

  // 3.5 LINE 推播客人付款確認（有 line_user_id 才推；沒有就 skip 不算錯）
  try {
    const r = await sendLinePush(v.lineUserId, [
      { type: "text", text: buildRenewalPaidLineText(v) },
    ]);
  ...
  try {
    ...
    if (report.line && report.line.errors.length)
