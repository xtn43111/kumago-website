 ...
 *      email?, lineUserId?, lineName?, note?, lang?} → Stripe metadata 物件。 */
...
function itemNameFor(v) {
  // 選填覆蓋：不用品處理等非「期滿回收」場景，老闆可自訂結帳頁品名（原樣顯示給客人）。
  const custom = String(v.itemName || "").trim();
...
function parseRecoveryLinkCommand(text, todayISO, now) {
  ...
  const LABELS = {
    ...
    "期限": "expires", "電話": "phone", "email": "email", "信箱": "email",
    "備註": "note",
  };
  for (const line of rest.split("\n")) {
    const m = line.match(/^\s*(姓名|金額|回收日|日期|地址|品項|電梯|期限|電話|email|信箱|備註)\s*[:：]\s*(.*)$/i);
    if (m) fields[LABELS[m[1].toLowerCase()]] = m[2].trim();
  }
  ...
  return {
    ...
    value: {
      ...
      email, note: fields.note || "", lang: "zh",
    },
  };
}
